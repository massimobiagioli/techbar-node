...
module.exports.create = function(models) {
	...
	// DEMO - Tutti i partecipanti degli eventi in Apra (nominaivo e presenze)
	var mapReduceDemo = function(request, response) {		
		var o = {};
		
		o.scope = {
			ref: request.query.where || 'Apra Informatica Jesi'
		};
		
		o.map = function() {
			var where = this.where;
			this.partecipants.forEach(function(p) {
				emit(p.name, where === ref ? 1 : 0);
			});
		};
		...
		models.Event.mapReduce(o, function(err, results, stats) {
			handleResponse(response, err, results);
		});
	};
